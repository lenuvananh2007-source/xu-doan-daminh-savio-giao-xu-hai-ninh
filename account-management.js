...
window.editAccountModal=function(payload){
 ...
 modal.innerHTML=`<div class="auth-box" style="width:min(820px,96vw);max-height:90vh;overflow:auto;padding:26px 28px">
   ...
   <div style="margin-bottom:18px;padding-right:36px">
     <div style="font-size:13px;font-weight:800;color:#2b64a3;text-transform:uppercase;letter-spacing:.04em">Quản trị tài khoản</div>
     <h2 style="margin:4px 0 0">${esc(a.full_name||'Tài khoản')}</h2>
     <div style="color:#748196;margin-top:5px">Chỉnh sửa tên đăng nhập, nhóm tài khoản và quyền truy cập</div>
   </div>
   <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px 18px">
     <label style="display:grid;gap:6px;font-weight:700;color:#26364d">Tên đăng nhập<input id="accEmail" class="input" value="${esc(a.email)}" style="width:100%;box-sizing:border-box"></label>
     <label style="display:grid;gap:6px;font-weight:700;color:#26364d">Nhóm tài khoản<select id="accGroup" class="select" style="width:100%;box-sizing:border-box">${opts.map(x=>`<option ${x===a.account_group?'selected':''}>${x}</option>`).join('')}</select></label>
   </div>
   ...
   <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:20px;position:sticky;bottom:-26px;background:#fff;padding:14px 0 4px">
     <button class="secondary" type="button" onclick="document.getElementById('accountAdminModal')?.remove()">Đóng</button>
     <button class="primary" type="button" onclick="saveAccountAdmin('${a.member_id}')">💾 Lưu thay đổi</button>
   </div>
